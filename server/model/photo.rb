...
class Photo < Sequel::Model
	...
	def self.do_create_photo_renditions(path, crop_to_landscape)
		...
		str = "convert #{path} -auto-orient -strip #{resize_params} #{path + Photo.get_rendition_suffix('original')};" \
			...
			"\\( +clone -resize 800x800^ -gravity center -extent 800x800  -write #{path + Photo.get_rendition_suffix('thumb-medium')} +delete \\) " \
